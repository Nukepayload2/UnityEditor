...
Structure AssetBundleFile
    ...
    Public Structure AssetBundleEntry
        ...
        End Sub
    End Structure
    Public Structure SizePair(Of T)
        <FieldDisplayName("在文件里的大小")>
        Dim Size As T
        <FieldDisplayName("未压缩时的大小")>
        Dim UncompressedSize As T
        Sub New(A As T, B As T)
            Size = A
            UncompressedSize = B
        End Sub
    End Structure
    ...
    Structure AssetBundleHeader
        ...
        Dim LevelByteEnd As List(Of SizePair(Of UInteger))
        ...
        Sub New(Strm As Stream)
            With New StreamHelper.BigendianBinaryReader(Strm)
                ...
                LevelByteEnd = New List(Of SizePair(Of UInteger))(NumberOfLevels)
                For i As Integer = 0 To NumberOfLevels - 1
                    LevelByteEnd.Add(New SizePair(Of UInteger)(.ReadUInt32, .ReadUInt32))
                Next
    ...
    Structure Asset
        ...
        Structure AssetMeta
            ...
            Sub New(Strm As Stream, RelevantHeader As AssetHeader, HeaderAssetAssetDataOffset As UInteger)
                Reversion = New StreamHelper.BigendianBinaryReader(Strm).ReadSinglebyteString(7).ToString
                With New BinaryReader(Strm)
                    TypetreeAttribute = .ReadInt32
        ...
        Structure ObjectData
            ...
            Sub New(Strm As Stream, RelevantHeader As ObjectHeader, HeaderAssetAssetDataOffset As UInteger)
                With New BinaryReader(Strm)
